...
(class Timer of Object
   ...
   ;;; twice as long as one with a fast machine.
	...
	(properties
		cycleCnt -1	;number of animation cycles left
	...
	(procedure (CueClient &tmp who)
		...
		(= client 0)
		(if (IsObject who)
			(if (who respondsTo: #timer) (who timer: 0))
			(if (who respondsTo: #cue) (who cue:))
		)
	)
	...
	(method (set call sec min hr &tmp aTimer theTicks theSpeed [temp3 50])
		...
		(if (> argc 2)
			(= theTicks (+ theTicks (/ (* min ticksPerMin) theSpeed)))
		)
		(if (> argc 3)
	...
	(method (setReal caller sec min hr &tmp aTimer secs)
		...
		(= secs sec)
		(if (> argc 2) (= secs (+ secs (* min 60))))
		(if (> argc 3) (= secs (+ secs (* hr 3600))))
		((= aTimer (if (& -info- CLASS) (self new:) else self))
			init: caller
			seconds: secs
		)
